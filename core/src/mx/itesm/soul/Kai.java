...
public class Kai extends Objeto {
    ...
    private void moverHorizontal(float delta, TiledMap mapa, OrthographicCamera camara) {
        ...
        if ( estadoMovimiento==EstadoMovimiento.MOV_DERECHA) {
            ...
            if ( celdaDerecha==null) {
                ...
                if((camara.position.x+ mx.itesm.soul.Pantalla.ANCHO/2) > (sprite.getX()+sprite.getWidth()))
                    sprite.setX(nuevaX);
            }
        }
    ...
    public boolean esAlcanzado(OrthographicCamera camara) {
        return (camara.position.x- mx.itesm.soul.Pantalla.ANCHO/2+sprite.getWidth()/2) >= (sprite.getX()+sprite.getWidth());
    }
